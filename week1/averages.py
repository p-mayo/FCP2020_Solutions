...
def main(*arguments):
    ...
    from_file = '--file' in flags # ['--mean', '--mode', '--file']

    if any(f in flags for f in ['--mean', '--mode', '--median']):
        #aux = []
        #for f in ['--mean', '--mode', '--median']:
        #    aux.append(f in flags) # [True, False, False]
        # mean, median and mode default to False if any are given
        # any(aux)
        # explicitly
        options = {
            'show_mean': '--mean' in flags, # True
            'show_median': '--median' in flags, # False
            'show_mode': '--mode' in flags, # False
        }
    else:
